...
from utils import atlas_labels, anatomical_classes, models, label_mapping
...
def compute_gradcam(model, images, target_layer, target='logit_diff', class_idx=None, mode='magnitude'):
    """
    ...
    try:
        ...
        if target == 'logit_diff' and logits.shape[1] == 2:
            ...
            if logits.dim() > 1 and logits.shape[1] == 1:
                ...
                raise ValueError("target='output' expects regression output shape [B] or [B,1]")
            score = out.sum()
        ...
        if mode == 'magnitude':
            # Normalize to [0, 1]
            cam = _normalize_batch_maps(cam, mode='magnitude')
        else:  # signed
            cam = _normalize_batch_maps(cam, mode='signed')
        
        return cam.detach().cpu().numpy(), pred_classes.detach().cpu().tolist(), confs.detach().cpu().tolist()
...
def compute_saliency(model, images, target='logit_diff', class_idx=None, mode='magnitude'):
    """
    ...
    return saliency.detach().cpu().numpy(), pred_classes.detach().cpu().tolist(), confs.detach().cpu().tolist()


def compute_inputxgradient(model, images, target='logit_diff', class_idx=None, mode='magnitude'):
    """
    ...
    return att.detach().cpu().numpy(), pred_classes.detach().cpu().tolist(), confs.detach().cpu().tolist()
...
def generate_heatmaps(heatmap_dir, attention_method='gradcam', 
    ...
    do_quantify = bool(getattr(cfg, "QUANTIFY_REGIONS", True)) and bool(atlas_path) and bool(label_dict)
    
    print("\nGenerating heatmaps...")
    stop_after_single = False
    for eids, images, labels in tqdm(test_loader, total=len(test_loader), desc="Processing"):
        images = images.to(device)

        # Compute attention for full batch
        if attention_method == 'gradcam':
            att_maps, pred_classes, confidences = compute_gradcam(
                model, images, target_layer,
